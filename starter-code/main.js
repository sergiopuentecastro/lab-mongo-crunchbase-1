      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          case "11":
            ...
            db.collection('companies').find({$and: [{name: 'Facebook'}, { "relationships.is_past": true}]}).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "13":
            ...
            case "15":
              ...
              db.collection('companies').find({ $and: [{ founded_year: { $gte: 2002 } }, { founded_year: { $lte: 2016 } }, {tag_list:{$regex: /social-network/}}]} ).count((error, result) => {
                if (error) {
                  console.log(error);
                  rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                } else {
            ...
            case "16":
              ...
              db.collection('companies').find({'offices.city':{$regex: "London"}}, {name:1,'offices.city':1 }).toArray((error, result) => {
                if (error) {
                  console.log(error);
                  rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                } else {
